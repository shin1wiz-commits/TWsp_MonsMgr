...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v5
...
def parse_explicit_four_body_row(
    ...
    if "4体配合" not in raw_expr:
        return None

    # 【4体配合】等のラベルを除去して親式だけにする
    cleaned = raw_expr
    cleaned = cleaned.replace("【4体配合】", " ")
    cleaned = re.sub(r"4体配合(?:の方法はこちら)?", " ", cleaned)
    cleaned = recipe_only_text(cleaned)
    cleaned = clean_text(cleaned)

    # × で厳密に4親へ分割
    parents = [clean_parent_phrase(x) for x in cleaned.split("×")]
    parents = [re.sub(r"^[・:：\s]+|[・:：\s]+$", "", p) for p in parents]
