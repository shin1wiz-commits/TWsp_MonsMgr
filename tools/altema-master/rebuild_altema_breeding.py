...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v8
...
def parse_pair_from_row(
    ...
    # 4) UNKNOWN
    # ページ主対象を子として推測しない。
    # 左セルが明示モンスター、右セルが配合式のときだけ暫定解析し REVIEW。
    if context == SOURCE_UNKNOWN and left_name in name_to_no and "×" in right_expr:
        cands = parse_child_recipe_expression(
            left_name, right_expr, name_to_no, source_url,
            original_joined, SOURCE_UNKNOWN
        )
        for c in cands:
            c.confidence = "REVIEW"
            c.note = "v8 UNKNOWN文脈: 左セル=子として暫定解釈"
        return cands
