...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v14
...
├─ altema_breeding_v14_boundary_focus.csv
...
def direct_boundary_index_v14(raw_section: str, result_name: str) -> int:
    """
    ページ主対象の direct 配合情報が終わり、
    「Xまでの作成ルート」へ移る最初の境界を返す。
    見つからなければ -1。
    """
    t = normalize_rule_text(raw_section)
    candidates = []
...
def candidate_crosses_creation_route_boundary_v14(
    ...
    """
    v14:
    direct候補ではない候補について、その根拠が作成ルート境界より後ろにしか
    現れない場合は「主対象の直接配合」として採用しない。

    source_context == DIRECT は v13 で切り出した領域なので対象外。
    """
    if getattr(cand, "source_context", SOURCE_UNKNOWN) == SOURCE_DIRECT:
        return False

    boundary = direct_boundary_index_v14(raw_section, result_name)
    if boundary < 0:
        return False
    ...
    parents = [
        clean_text(getattr(cand, "parent1", "") or ""),
        clean_text(getattr(cand, "parent2", "") or ""),
        clean_text(getattr(cand, "parent3", "") or ""),
        clean_text(getattr(cand, "parent4", "") or ""),
    ]
    parents = [p for p in parents if p]

    # 「相手問わず」型は特に作成ルート途中から混入しやすい。
    if "相手問わず" in parents:
        named = [p for p in parents if p != "相手問わず"]
        if named:
            before_has = any(p in before for p in named)
            after_has = any(p in after for p in named)
            if after_has and not before_has:
                return True

    # 系統指定・通常親も、全親が境界後にしか現れないなら境界跨ぎ扱い。
    meaningful = [p for p in parents if p != "相手問わず"]
    if meaningful:
        all_after = all(p in after for p in meaningful)
        none_before = all(p not in before for p in meaningful)
        if all_after and none_before:
            return True
...
def parse_creation_section(
    ...
    for cand in row_candidates:
        if is_ui_fragment_candidate_v13(cand):
            continue
        if candidate_crosses_creation_route_boundary_v14(
            cand, raw_section, expected_name
        ):
            continue
...
def main():
    ...
    # ---- v14: 作成ルート境界の重点検証 ----
    # MEDIUM/REVIEWだけでなく、境界関連の注記を持つ候補も確認可能にする。
    boundary_focus_v14 = []
    for r in all_rows:
        note = str(r.get("note", ""))
        raw = str(r.get("raw_text", ""))
        if (
            r.get("confidence") in {"MEDIUM", "REVIEW"}
            or "境界" in note
            or "作成ルート" in raw
        ):
            boundary_focus_v14.append({
                "result_no": r.get("result_no",""),
                "result_name": r.get("result_name",""),
                "recipe_type": r.get("recipe_type",""),
                "parent1": r.get("parent1",""),
    ...
    write_csv(
        OUT / "altema_breeding_v14_boundary_focus.csv",
        boundary_focus_v14,
        [
