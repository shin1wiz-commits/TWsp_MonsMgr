...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v28改
...
def semantic_false_positive_cleanup_v28_kai(rows: list[dict]):
    ...
    C群で確認された誤取得は、結果名そのもの（配合早見表の見出し）に
    含まれる既知モンスター名を、同一親×同一親として拾ったもの。
    例: スライムベス -> スライム×スライム。

    固有名のハードコードではなく、以下をすべて満たす場合だけREJECTする。
      - 2体配合
      - v13 ADDITIONAL direct由来
      - 両親が同じ具体モンスター
      - 親名が結果名の真部分文字列
      - raw_textに実配合を示す「配合パターン」「への配合」が無い

    これにより、ヘルバトラー <- アンクルホーン×アンクルホーンのように
    raw_text上で実際の配合パターンが確認できる候補は除外しない。
    """
    kept, rejected = [], []
    for src in rows:
        r = dict(src)
        ...
        is_heading_substring_fp = (
            typ == "2体配合"
            and "v13 ADDITIONAL direct / 2体配合" in note
            and p1 and p1 == p2
            and p1 != result and p1 in result
            and "配合パターン" not in raw
            and "への配合" not in raw
        )

        if is_heading_substring_fp:
            r["v28k_action"] = "REJECT"
            r["v28k_reason"] = "HEADING_SUBSTRING_FALSE_POSITIVE"
            rejected.append(r)
        else:
            r["v28k_action"] = "KEEP"
            r["v28k_reason"] = ""
...
def main():
    ...
    write_csv(
        OUT / "altema_breeding_v28k_semantic_rejected.csv",
        semantic_rejected_v28k,
        semantic_v28k_fields,
    )
    write_csv(
        OUT / "altema_breeding_v28k_cleanup_summary.csv",
        [{
            "v27_master_count": semantic_stats_v27["v27_final_master_count"],
            "v28k_rejected_count": len(semantic_rejected_v28k),
            "v28k_final_master_count": len(all_rows),
        }],
        ["v27_master_count", "v28k_rejected_count", "v28k_final_master_count"],
    )

    # ---- v28監査は、v28改で確定C群を除外した後の候補に対して継続 ----
