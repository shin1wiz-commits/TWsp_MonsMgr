...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール
...
def parse_pair_from_row(
    ...
    texts = [normalize_rule_text(node_text(c)) for c in cells]
    joined = " | ".join(texts)
...
def parse_pair_from_text_line(
    ...
    「A × B」を1行テキストから解析する。
    """
    t = normalize_rule_text(text)
    if "×" not in t:
        return None

    # 作成ルート説明に混ざる長文は避ける。
    if len(t) > 80 or "入手方法" in t or "位階配合" in t or "特殊配合" in t:
        return None
...
def parse_creation_section(
    ...
    if not any(x.confidence == "HIGH" for x in found):
        for s in sec.stripped_strings:
            cand = parse_pair_from_text_line(
