...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v20
...
# v20: 主配合表の正式構造解析
# ============================================================
def _v20_candidate(expected_no, expected_name, recipe_type, parents,
                   source_url, raw_text, method):
    p = list(parents)[:4]
    p += [""] * (4-len(p))
    return RecipeCandidate(
        result_no=expected_no,
        ...
        is_fallback=False,
        note="v20: main breeding table structurally confirmed",
        raw_text=raw_text,
        source_url=source_url,
    )

def parse_main_breeding_table_v20(sec, expected_no, expected_name, name_to_no, source_url):
    tables = sec.select("table.tableLine")
    if not tables:
        return []

    # 「対象名への配合ルート/配合パターン」を含む最初のtableLineを主表とする
    main = None
    for t in tables:
        tt = clean_text(t.get_text(" ", strip=True))
        if expected_name in tt and ("配合ルート" in tt or "配合パターン" in tt or "4体配合" in tt):
            main = t
            break
    if main is None:
        return []
    ...
    full = clean_text(main.get_text(" ", strip=True))

    # B: 配合パターン①②③型
    pattern_headers = []
    for ri,tr in enumerate(trs):
        cells=tr.find_all(["td","th"], recursive=False)
        if any("配合パターン" in clean_text(c.get_text(" ",strip=True)) for c in cells):
            pattern_headers.append(ri)

    if pattern_headers:
        out=[]
        for hi,ri in enumerate(pattern_headers):
            hdr_cells=trs[ri].find_all(["td","th"], recursive=False)
            spans=[]; col=0
            for c in hdr_cells:
                ...
                col+=span
            endrow=pattern_headers[hi+1] if hi+1<len(pattern_headers) else len(trs)
            for label,start,end in spans:
                names=[]; raw=[label]
                for rr in trs[ri+1:endrow]:
                    ccol=0
                    for c in rr.find_all(["td","th"], recursive=False):
                        span=int(c.get("colspan",1) or 1)
                        a,b=ccol,ccol+span; ccol=b
                        if b<=start or a>=end:
                            continue
                        ct=clean_text(c.get_text(" ",strip=True))
                        raw.append(ct)
                        for n in known_monsters_in_text_order(ct,name_to_no,exclude={expected_name}):
                            if n not in names:
                                names.append(n)
                if len(names)>=2:
                    out.append(_v20_candidate(expected_no,expected_name,"2body",names[:2],
                                              source_url," | ".join(raw),"v20_main_multi_pattern"))
        if out:
            return out
    ...
    if "4体配合" in full:
        ...
        for tr in trs:
            ...
            names.extend(known_monsters_in_text_order(tt,name_to_no,exclude={expected_name}))
        if len(names)>=4:
            return [_v20_candidate(expected_no,expected_name,"4body",names[-4:],
                                   source_url,full,"v20_main_4body")]
    ...
    for tr in trs:
        ...
        if "相手問わず" in tt:
            ...
            if names:
                return [_v20_candidate(expected_no,expected_name,"2body",
                                       [names[0],"相手問わず"],source_url,tt,
                                       "v20_main_2body_any")]
    ...
    for tr in trs:
        ...
        names=known_monsters_in_text_order(tt,name_to_no,exclude={expected_name})
        if len(names)>=2:
            return [_v20_candidate(expected_no,expected_name,"2body",names[:2],
                                   source_url,tt,"v20_main_2body_fixed")]
    return []
...
def parse_creation_section(
    ...
    # v20: 主配合表で確定できたら補完処理へ進まない
    v20_main = parse_main_breeding_table_v20(
    ...
    if v20_main:
        return v20_main, {
            ...
            "reason": "v20 main table confirmed",
            "identity_ok": name_ok and no_ok,
            "v20_main_confirmed": True,
            "v20_fallback_suppressed": True,
            "dom_probe_v17": [],
            "direct_candidates_v17": [],
            "tableline_audit_v18": [],
            "group_a_probe_v19": [],
            "group_b_probe_v19": [],
            "group_c_probe_v19": [],
