...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v12
...
def parse_creation_section(
    ...
    # 0) v9: 主対象の「Xへの配合パターン」を raw_section から最優先取得
    ...
    # v12: 「その他の配合パターン」は主対象の追加配合として独立解析。
    for additional_seg in extract_additional_direct_sections(raw_section, expected_name):
        extra = parse_direct_pattern_from_raw_section(
            additional_seg, expected_no, expected_name, name_to_no, source_url
        )
        for cand in extra:
            cand.note = ("v12 その他の配合パターン | " + cand.note).strip(" |")
            cand.source_context = SOURCE_DIRECT
