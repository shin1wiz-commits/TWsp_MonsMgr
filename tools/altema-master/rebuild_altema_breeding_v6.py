...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v6
...
├─ altema_breeding_v6_four_body_normalized.csv
...
def extract_four_body_from_ui_table(
    ...
    return build_candidate(
        ...
        raw_text=table_text,
        recipe_type="4体配合",
        note="v6 4体配合UIの4枠を直接取得",
        confidence="HIGH",
    )


def four_body_signature(r: dict) -> tuple:
    return (
        int(r.get("result_no") or 0),
        tuple(sorted([
            clean_text(r.get("parent1","")),
            clean_text(r.get("parent2","")),
            clean_text(r.get("parent3","")),
            clean_text(r.get("parent4","")),
        ]))
    )
...
def parse_pair_from_row(
    ...
    if four:
        # v6: 4体配合tableと判定できたら通常2体配合解析へ流さない
        return [four]
    ...
    # v5: 明示4体配合は通常2体配合より必ず先に処理し、
    # 成功したらこの行は以降の通常解析へ流さない。
    if left_name in name_to_no and "4体配合" in originals[1]:
        ...
        # 明示4体配合なのに安全に4親へ分解できない場合は
        # 2体配合へ誤変換せず REVIEW として残す。
        return [RecipeCandidate(
            result_no=name_to_no[left_name],
            result_name=left_name,
            parent1_type="review", parent1_no="", parent1=originals[1],
            parent2_type="review", parent2_no="", parent2="",
            recipe_type="4体配合",
            confidence="REVIEW",
            note="v5 明示4体配合だが4親へ安全に分解できない",
            source_url=source_url,
...
def main():
    ...
    for r in all_rows:
        ...
        v6_rows.append({
            ...
            "normalized_signature": " | ".join(sorted([
                clean_text(r.get("parent1","")),
                clean_text(r.get("parent2","")),
                clean_text(r.get("parent3","")),
                clean_text(r.get("parent4","")),
    ...
    write_csv(
        OUT / "altema_breeding_v6_four_body_normalized.csv",
        v6_rows,
        [
            "result_no","result_name",
            "parent1","parent2","parent3","parent4",
            "normalized_signature","confidence","note","source_url","raw_text"
        ],
    )
