...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合データ 最終統合版（v26〜v27仕様準拠 / v28改2＋最終確定）
...
重要
----
このツールは v26〜v27 と同じ実行仕様で659体の個別ページを直接解析し、
v26 → v27 → v28改2 → 人手確認済み最終確定処理までを1回で実行します。
既存 app ZIP 内の breeding.csv 自体は書き換えません。
最終確定した breeding.csv を output_breeding/ に出力します。
...
├─ altema_breeding_v26_all_observations.csv
├─ altema_breeding_v26_rejected_malformed.csv
├─ altema_breeding_v26_provenance.csv
├─ altema_breeding_v26_cleanup_summary.csv
...
├─ altema_breeding_v19_groupA_normal2body_probe.csv
├─ altema_breeding_v19_groupB_multi_pattern_probe.csv
├─ altema_breeding_v19_groupC_4body_fallback_probe.csv
├─ altema_breeding_v19_remaining6_current_state.csv
├─ altema_breeding_v21_focus6_validation.csv
├─ altema_breeding_v22_multi_pattern_validation.csv
├─ altema_breeding_v22_dohk_probe.csv
├─ altema_breeding_v22_focus_validation.csv
├─ altema_breeding_v23_dohk_dom_probe.csv
├─ altema_breeding_v23_dohk_table_probe.csv
├─ altema_breeding_v24_dohk_page_trace.csv
├─ altema_breeding_v24_dohk_heading_trace.csv
...
def clean_text(v) -> str:
    """
    文字列正規化。
    CSV/内部処理では result_no などが int で渡る場合があるため、
    None・数値・文字列のどれでも安全に扱う。
    """
    if v is None:
        v = ""
...
def get(url: str, tries: int = 4) -> str:
    """
    v25:
    Altema HTMLをHTTPレスポンスのbytesからUTF-8で明示decodeする。
    自動文字コード推定による日本語文字化けを防ぐ。
    """
    last = None
...
def find_creation_heading(soup: BeautifulSoup, monster_name: str) -> Tag | None:
    """
    v22:
    「○○の配合表・作り方」に相当する見出しを探す。
    h2/h3/h4に限定せず、h1-h6を優先し、それでも無ければ
    見出し風ブロックも補助的に探す。
    """
    best = None
    ...
    for h in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6"]):
        ...
        if "使う主な配合" in t:
            continue
        if ("配合表" in t or "配合パターン" in t) and (
            "作り方" in t or monster_name in t
        ):
            best = h
            if "作り方" in t:
                return h
    ...
    # Altema側のHTML表現揺れ用の保険。
    # テキストが短い見出し風ブロックだけを対象にする。
    for tag in soup.find_all(["div", "p", "span", "strong"]):
        ...
        if "使う主な配合" in t:
            continue
        if monster_name in t and "配合表" in t and "作り方" in t:
            return tag
...
V19_GROUP_A = {"コドラ", "ミステリドール"}
V19_GROUP_B = {"プチットガールズ", "モヒカント"}
V19_GROUP_C = {"キラーパンサー", "トラップボックス"}

def classify_v19_group(name: str) -> str:
    if name in V19_GROUP_A:
        return "A_NORMAL_2BODY_LEAK"
    if name in V19_GROUP_B:
        return "B_MULTI_PATTERN"
    if name in V19_GROUP_C:
        return "C_4BODY_WITH_FALLBACK"
    return ""

def probe_group_a_v19(sec, expected_no, expected_name, name_to_no, source_url):
    rows = []
    if expected_name not in V19_GROUP_A:
        return rows
    for ti, table in enumerate(sec.select("table.tableLine"), 1):
        for ri, tr in enumerate(table.find_all("tr")):
            txt = clean_text(tr.get_text(" ", strip=True))
            names = known_monsters_in_text_order(txt, name_to_no, exclude={expected_name})
            fam = canonical_family_from_text_v18(txt)
            anyp = "相手問わず" in txt
            rows.append({
                "group": "A_NORMAL_2BODY_LEAK",
                "result_no": expected_no,
                "result_name": expected_name,
                "table_index": ti,
                "row_index": ri,
                "table_class": " ".join(table.get("class", [])),
                "row_tag": getattr(tr, "name", "") or "",
                "row_class": " ".join(tr.get("class", [])),
                "row_text": txt,
                "known_monsters": " | ".join(names),
                "family": fam,
                "any_partner": "YES" if anyp else "NO",
                "link_count": len(tr.find_all("a", href=True)),
                "img_count": len(tr.find_all("img")),
                "source_url": source_url,
            })
    return rows

def probe_group_b_v19(sec, expected_no, expected_name, name_to_no, source_url):
    rows = []
    if expected_name not in V19_GROUP_B:
        return rows
    for ti, table in enumerate(sec.select("table.tableLine"), 1):
        trs = table.find_all("tr")
        for ri, tr in enumerate(trs):
            txt = clean_text(tr.get_text(" ", strip=True))
            if "配合パターン" not in txt:
                continue
            for offset in range(0, 4):
                ...
                if idx >= len(trs):
                    break
                rr = trs[idx]
                rr_txt = clean_text(rr.get_text(" ", strip=True))
                names = known_monsters_in_text_order(rr_txt, name_to_no, exclude={expected_name})
                rows.append({
                    "group": "B_MULTI_PATTERN",
                    "result_no": expected_no,
                    "result_name": expected_name,
                    "table_index": ti,
                    "pattern_header_row": ri,
                    "relative_offset": offset,
                    "row_index": idx,
                    "row_text": rr_txt,
                    "known_monsters": " | ".join(names),
                    "cell_count": len(rr.find_all(["td","th"], recursive=False)),
                    "link_count": len(rr.find_all("a", href=True)),
                    "source_url": source_url,
                })
    return rows

def probe_group_c_v19(sec, expected_no, expected_name, name_to_no, source_url):
    rows = []
    if expected_name not in V19_GROUP_C:
        return rows

    for ti, table in enumerate(sec.select("table.tableLine"), 1):
        ttxt = clean_text(table.get_text(" ", strip=True))
        names = known_monsters_in_text_order(ttxt, name_to_no, exclude={expected_name})
        rows.append({
            "group": "C_4BODY_WITH_FALLBACK",
            "result_no": expected_no,
            "result_name": expected_name,
            "table_index": ti,
            "table_text": ttxt[:1200],
            "known_monsters": " | ".join(names),
            "has_4body_marker": "YES" if "4体配合" in ttxt else "NO",
            "has_any_partner": "YES" if "相手問わず" in ttxt else "NO",
            "source_url": source_url,
        })

    for di, dl in enumerate(sec.select("dl.acMenu"), 1):
        txt = clean_text(dl.get_text(" ", strip=True))
        if "相手問わず" not in txt:
            continue
        names = known_monsters_in_text_order(txt, name_to_no, exclude={expected_name})
        rows.append({
            ...
            "result_no": expected_no,
            "result_name": expected_name,
            "table_index": f"acMenu:{di}",
            "table_text": txt[:1200],
            "known_monsters": " | ".join(names),
            "has_4body_marker": "YES" if "4体配合" in txt else "NO",
            "has_any_partner": "YES",
            "source_url": source_url,
        })
    return rows
...
# ============================================================
# v21
# v19を基準に、主配合表を「追加候補」として解析する。
# 早期returnはしない。
# 既存v19解析を最後まで実行した後、主配合表候補を統合し、
# 主対象directが確定した場合のみ誤fallbackを除去する。
# ============================================================

def _v21_parent_token(name, name_to_no):
    if name == "相手問わず":
        return ("rule", "", name)
    # family canonical values
    if name in set(FAMILY_DISPLAY_ALIASES_V16.values()):
        return ("family", "", name)
    return make_parent_token(name, name_to_no)


def _v21_make_candidate(expected_no, expected_name, parents,
                        source_url, raw_text, method, recipe_type="2体配合"):
    ps = list(parents)[:4]
    ps += [""] * (4 - len(ps))

    toks = [_v21_parent_token(x, {}) if x else ("", "", "") for x in ps]
    # monster token needs actual no later; caller replaces via make_parent_token path
    return {
        "result_no": expected_no,
        "result_name": expected_name,
        "parents": ps,
...
def parse_main_table_candidates_v21(sec, expected_no, expected_name, name_to_no, source_url):
    """
    主対象の table.tableLine を構造解析。
    戻り値は RecipeCandidate の追加候補。既存処理は止めない。
    """
    out = []

    tables = sec.select("table.tableLine")
    for table in tables:
        rows = table.find_all("tr")
        texts = [clean_text(r.get_text(" ", strip=True)) for r in rows]
        full = clean_text(table.get_text(" ", strip=True))

        # 主対象表であることを複数条件で確認
        has_title = any(
            expected_name in t and ("配合早見表" in t or "配合表" in t)
            for t in texts[:3]
        )
        has_child = any(t == expected_name for t in texts)
        if not (has_title or has_child):
            continue
        ...
        for ri, tr in enumerate(rows):
            ...
            if any("配合パターン" in x for x in labels):
                pattern_rows.append(ri)

        multi = []
        if pattern_rows:
            for hpos, ri in enumerate(pattern_rows):
                header_cells = rows[ri].find_all(["td", "th"], recursive=False)
                spans = []
                col = 0
                for cell in header_cells:
                    ...
                    if "配合パターン" in label:
                        spans.append((label, col, col + span))
                    col += span

                end_ri = pattern_rows[hpos + 1] if hpos + 1 < len(pattern_rows) else len(rows)

                for label, start, end in spans:
                    names = []
                    raw_parts = [label]
                    for rr in rows[ri + 1:end_ri]:
                        ccol = 0
                        for cell in rr.find_all(["td", "th"], recursive=False):
                            span = int(cell.get("colspan", 1) or 1)
                            a, b = ccol, ccol + span
                            ccol = b
                            if b <= start or a >= end:
                                continue
                            ct = clean_text(cell.get_text(" ", strip=True))
                            raw_parts.append(ct)
                            for n in known_monsters_in_text_order(
                                ct, name_to_no, exclude={expected_name}
                            ):
                                if n not in names:
                                    names.append(n)
        ...
        # ---------- 4体配合型 ----------
        if "4体配合" in full:
            # v19既存4体parserを優先するため、ここでは追加生成せず
            # 「主表が4体配合である」ことだけ後段のfallback抑止材料にする。
            continue

        # ---------- 通常2体型 ----------
        # 「配合パターン/配合ルート」見出しの次の実データ行を優先
        header_idx = None
        for i, t in enumerate(texts):
            if "配合パターン" in t or "配合ルート" in t:
                header_idx = i
                break

        candidate_rows = rows[header_idx + 1:] if header_idx is not None else rows
        for tr in candidate_rows:
            tt = clean_text(tr.get_text(" ", strip=True))
            if not tt:
                continue

            names = known_monsters_in_text_order(tt, name_to_no, exclude={expected_name})
            fam = canonical_family_from_text_v18(tt)
            anyp = "相手問わず" in tt
            ...
            if names and fam:
                ...
                out.append(RecipeCandidate(
                    result_no=expected_no, result_name=expected_name,
                    parent1_type=p1[0], parent1_no=p1[1], parent1=p1[2],
                    parent2_type="family", parent2_no="", parent2=fam,
                    recipe_type="2体配合", confidence="HIGH",
                    note="v21 主配合表 monster×family",
                    source_url=source_url, source_context=SOURCE_DIRECT,
                    source_method=METHOD_DIRECT_STRUCTURED,
                    is_fallback=False, fallback_reason="", raw_text=tt,
                ))
                break
    ...
    uniq = []
    for c in out:
        if not any(same_recipe_ordered(c, x) for x in uniq):
            uniq.append(c)
    return uniq


def main_table_is_confirmed_v21(sec, expected_name):
    """
    fallback抑止用。
    主配合表が2体/複数パターン/4体のいずれかとして明示されているか。
    """
    for table in sec.select("table.tableLine"):
        texts = [clean_text(r.get_text(" ", strip=True)) for r in table.find_all("tr")]
        full = clean_text(table.get_text(" ", strip=True))
        if not any(expected_name in t for t in texts[:3]) and expected_name not in full[:300]:
            continue
        if (
            "配合パターン" in full
            or "配合ルート" in full
...
V22_NO_BREEDING_KNOWN = {"スライム", "ズッキーニャ", "キラーピッケル"}

def page_declares_no_breeding_v22(soup: BeautifulSoup, expected_name: str) -> bool:
    """
    「作る配合が無い」を正常0件として判定する。
    基本情報の 配合 × を最優先。既知3例も回帰確認用に保持。
    """
    # 基本情報テーブルの「配合 | ×」を探す
    for tr in soup.find_all("tr"):
        cells = [node_text(c) for c in tr.find_all(["th", "td"])]
        for i, c in enumerate(cells[:-1]):
            if c == "配合" and cells[i + 1] in ("×", "✕", "x", "X"):
                return True

    # 既知の配合なし3体は安全側の回帰保証
    if expected_name in V22_NO_BREEDING_KNOWN:
        return True

    return False


def classify_pattern_cell_v22(cell, expected_name, name_to_no):
    """
    複数配合パターンの1セルを親tokenとして分類。
    プチット族のようにmasterに無い表現も rule として残す。
    """
    txt = clean_text(cell.get_text(" ", strip=True))
    if not txt or txt == expected_name:
        return None
    ...
    p = classify_parent_token_v16(txt, link_names, name_to_no)
...
def parse_main_multi_patterns_v22(
    sec, expected_no, expected_name, name_to_no, source_url
):
    """
    v22:
    配合パターン①/②/③を「仮想列span」単位で分け、
    monsterだけでなく rule/family 親も正式に取得する。
    """
    out = []
    ...
    for table in sec.select("table.tableLine"):
        ...
        # 主対象の表だけ
        if expected_name not in full[:500]:
            continue
        ...
        for ri, tr in enumerate(rows):
            cells = tr.find_all(["td", "th"], recursive=False)
            if any(
                "配合パターン" in clean_text(c.get_text(" ", strip=True))
                for c in cells
            ):
        ...
        for hpos, ri in enumerate(header_rows):
            header_cells = rows[ri].find_all(["td", "th"], recursive=False)

            spans = []
            col = 0
            for c in header_cells:
                label = clean_text(c.get_text(" ", strip=True))
            ...
            for label, start, end in spans:
                ...
                for rr in rows[ri + 1:end_ri]:
                    ...
                    for cell in rr.find_all(["td", "th"], recursive=False):
                        ...
                        p = classify_pattern_cell_v22(
                            cell, expected_name, name_to_no
                        )
                        if p is None:
                            continue

                        sig = (p[0], p[1], p[2])
                        if sig not in parent_tokens:
                            parent_tokens.append(sig)

                # 1パターン = 親2つ
                if len(parent_tokens) < 2:
                    continue

                p1, p2 = parent_tokens[0], parent_tokens[1]
                ...
                if not any(same_recipe_ordered(cand, x) for x in out):
                    out.append(cand)

    return out


def probe_dohk_v22(sec, expected_name, source_url):
    """
    ドーク型の MAIN + その他の配合パターン の構造監査。
    """
    if expected_name != "ドーク":
        return []
    ...
    for ti, table in enumerate(sec.select("table.tableLine"), 1):
        ...
        rows.append({
            "result_name": expected_name,
            "table_index": ti,
            "has_other_patterns": "YES" if "その他の配合パターン" in txt else "NO",
            "has_pattern": "YES" if "配合パターン" in txt else "NO",
            "table_text": txt[:1400],
            "source_url": source_url,
        })
    return rows
...
V23_DOHK_KEYWORDS = [
    "ドーク","配合表","作り方","配合パターン","その他の配合パターン",
    "アクバー","バルザック","ムドー","カンダタおやぶん","カンダタワイフ",
]

def _node_info_v23(node):
    if node is None:
        return {"tag":"","class":"","id":"","text":"","href":"","img_alt":""}
    tag = getattr(node, "name", "") or ""
    classes = " ".join(node.get("class", [])) if hasattr(node, "get") else ""
    node_id = node.get("id", "") if hasattr(node, "get") else ""
    txt = clean_text(node.get_text(" ", strip=True)) if hasattr(node, "get_text") else ""
    ...
    if hasattr(node, "find"):
        ...
        if img is not None:
            img_alt = img.get("alt", "") or ""
    return {"tag":tag,"class":classes,"id":node_id,"text":txt,"href":href,"img_alt":img_alt}

def probe_dohk_dom_v23(soup, expected_name, source_url):
    if expected_name != "ドーク":
        return []
    rows, seen = [], set()
    for node in soup.find_all(True):
        ...
        if not txt or len(txt) > 700:
            continue
        matched = [k for k in V23_DOHK_KEYWORDS if k in txt]
        if not matched:
            continue
        info = _node_info_v23(node)
        parent_node = getattr(node, "parent", None)
        grand_node = getattr(parent_node, "parent", None) if parent_node is not None else None
        parent = _node_info_v23(parent_node)
        grand = _node_info_v23(grand_node)
        prev = _node_info_v23(node.find_previous_sibling() if hasattr(node, "find_previous_sibling") else None)
        nxt = _node_info_v23(node.find_next_sibling() if hasattr(node, "find_next_sibling") else None)

        sig = (info["tag"], info["class"], info["id"], info["text"][:180], "|".join(matched))
        if sig in seen:
            continue
        seen.add(sig)
        ...
        rows.append({
            ...
            "tag": info["tag"], "class": info["class"], "id": info["id"],
            "text": info["text"][:700], "href": info["href"], "img_alt": info["img_alt"],
            "parent_tag": parent["tag"], "parent_class": parent["class"], "parent_id": parent["id"],
            "parent_text": parent["text"][:700],
            "grandparent_tag": grand["tag"], "grandparent_class": grand["class"], "grandparent_id": grand["id"],
            "grandparent_text": grand["text"][:700],
            "prev_tag": prev["tag"], "prev_class": prev["class"], "prev_text": prev["text"][:500],
            "next_tag": nxt["tag"], "next_class": nxt["class"], "next_text": nxt["text"][:500],
            "source_url": source_url,
        })
    return rows

def probe_dohk_tables_v23(soup, expected_name, source_url):
    if expected_name != "ドーク":
        return []
    rows = []
    for ti, table in enumerate(soup.find_all("table"), 1):
        ...
        for ri, tr in enumerate(table.find_all("tr")):
            ...
            rows.append({
                ...
                "table_matched_keywords": " | ".join(matched),
                "row_index": ri,
                "row_text": clean_text(tr.get_text(" ", strip=True))[:700],
                "cell_count": len(cells),
                "cell_texts": " || ".join(clean_text(c.get_text(" ", strip=True)) for c in cells)[:1200],
                "link_texts": " || ".join(clean_text(a.get_text(" ", strip=True)) for a in tr.find_all("a"))[:1200],
                "img_alts": " || ".join((img.get("alt","") or "") for img in tr.find_all("img"))[:1200],
                "source_url": source_url,
            })
    return rows
...
# ============================================================
# v24: ドーク /monster/564 取得→見出し→section トレース
# 取得結果そのものは変更しない診断版。
# ============================================================

V24_DOHK_TERMS = {
    "dohk": "ドーク",
    "creation_heading": "ドークの配合表・作り方",
    "akubar": "アクバー",
    "baruzakku": "バルザック",
    "mudo": "ムドー",
    "kandata_oyabun": "カンダタおやぶん",
    "kandata_wife": "カンダタワイフ",
    "other_patterns": "その他の配合パターン",
}

def trace_dohk_page_v24(html, soup, expected_name, source_url):
    if expected_name != "ドーク":
        return []
    raw_html = html or ""
    page_text = node_text(soup)
    page_no, title = page_identity(soup)
    heading = find_creation_heading(soup, expected_name)
    heading_found = heading is not None
    heading_text = node_text(heading) if heading is not None else ""
    section_text = ""
    section_length = 0
    if heading is not None:
        try:
            ...
            section_text = node_text(sec)
            section_length = len(section_text)
        except Exception as e:
            section_text = f"[SECTION_ERROR] {type(e).__name__}: {e}"
            section_length = -1
    return [{
        ...
        "contains_dohk_html": "YES" if V24_DOHK_TERMS["dohk"] in raw_html else "NO",
        "contains_dohk_text": "YES" if V24_DOHK_TERMS["dohk"] in page_text else "NO",
        "contains_creation_heading_html": "YES" if V24_DOHK_TERMS["creation_heading"] in raw_html else "NO",
        "contains_creation_heading_text": "YES" if V24_DOHK_TERMS["creation_heading"] in page_text else "NO",
        "contains_akubar_html": "YES" if V24_DOHK_TERMS["akubar"] in raw_html else "NO",
        "contains_baruzakku_html": "YES" if V24_DOHK_TERMS["baruzakku"] in raw_html else "NO",
        "contains_mudo_html": "YES" if V24_DOHK_TERMS["mudo"] in raw_html else "NO",
        "contains_kandata_oyabun_html": "YES" if V24_DOHK_TERMS["kandata_oyabun"] in raw_html else "NO",
        "contains_kandata_wife_html": "YES" if V24_DOHK_TERMS["kandata_wife"] in raw_html else "NO",
        "contains_other_patterns_html": "YES" if V24_DOHK_TERMS["other_patterns"] in raw_html else "NO",
        "contains_mojibake_hint": "YES" if ("г" in raw_html[:5000] or "Җ" in raw_html[:5000]) else "NO",
        "heading_found": "YES" if heading_found else "NO",
        "heading_tag": getattr(heading, "name", "") if heading is not None else "",
        "heading_class": " ".join(heading.get("class", [])) if heading is not None and hasattr(heading, "get") else "",
        "heading_id": heading.get("id", "") if heading is not None and hasattr(heading, "get") else "",
        "heading_text": heading_text[:500],
        "section_length": section_length,
        "section_contains_akubar": "YES" if V24_DOHK_TERMS["akubar"] in section_text else "NO",
        "section_contains_baruzakku": "YES" if V24_DOHK_TERMS["baruzakku"] in section_text else "NO",
        "section_contains_mudo": "YES" if V24_DOHK_TERMS["mudo"] in section_text else "NO",
        "section_contains_kandata_oyabun": "YES" if V24_DOHK_TERMS["kandata_oyabun"] in section_text else "NO",
        "section_contains_kandata_wife": "YES" if V24_DOHK_TERMS["kandata_wife"] in section_text else "NO",
        "section_contains_other_patterns": "YES" if V24_DOHK_TERMS["other_patterns"] in section_text else "NO",
        "html_prefix": raw_html[:1200].replace("\\n", " "),
        "page_text_prefix": page_text[:1200].replace("\\n", " "),
        "section_text_prefix": section_text[:1600].replace("\\n", " "),
    }]

def trace_dohk_headings_v24(soup, expected_name, source_url):
    if expected_name != "ドーク":
        return []
    rows=[]
    idx=0
    for node in soup.find_all(["h1","h2","h3","h4","h5","h6","div","p","span","strong"]):
        ...
        if not txt or len(txt)>140:
            continue
        if not any(k in txt for k in ["ドーク","配合表","作り方","配合パターン"]):
            continue
        idx+=1
        ...
        rows.append({
            ...
            "text": txt,
            "parent_tag": getattr(parent,"name","") if parent is not None else "",
            "parent_class": " ".join(parent.get("class",[])) if parent is not None and hasattr(parent,"get") else "",
            "parent_id": parent.get("id","") if parent is not None and hasattr(parent,"get") else "",
            "parent_text": clean_text(parent.get_text(" ",strip=True))[:700] if parent is not None and hasattr(parent,"get_text") else "",
            "source_url": source_url,
        })
    return rows
...
def parse_creation_section(
    ...
    # v24: ドーク /564 の取得→見出し→section 診断
    dohk_page_trace_v24 = trace_dohk_page_v24(html, soup, expected_name, source_url)
    dohk_heading_trace_v24 = trace_dohk_headings_v24(soup, expected_name, source_url)

    # v23: ドーク専用DOM診断。取得結果には影響しない。
    dohk_dom_probe_v23 = probe_dohk_dom_v23(soup, expected_name, source_url)
    dohk_table_probe_v23 = probe_dohk_tables_v23(soup, expected_name, source_url)
    ...
    heading = find_creation_heading(soup, expected_name)
    if heading is None:
        no_breeding_v22 = page_declares_no_breeding_v22(soup, expected_name)
        return [], {
            "status": "NO_BREEDING" if no_breeding_v22 else "NO_SECTION",
            "page_no": page_no or "",
            "title": title,
            "raw_section": "",
            "reason": (
            ...
            "group_b_probe_v19": [],
            "group_c_probe_v19": [],
            "main_confirmed_v21": False,
            "main_candidate_count_v21": 0,
            "fallback_suppressed_v21": False,
            "multi_pattern_v22": [],
            "dohk_probe_v22": [],
            "dohk_dom_probe_v23": dohk_dom_probe_v23,
            "dohk_table_probe_v23": dohk_table_probe_v23,
            "dohk_page_trace_v24": dohk_page_trace_v24,
            "dohk_heading_trace_v24": dohk_heading_trace_v24,
        }
    ...
    # v21: 主配合表候補を追加取得。ここでは処理を終了しない。
    main_candidates_v21 = parse_main_table_candidates_v21(
        sec, expected_no, expected_name, name_to_no, source_url
    )
    main_confirmed_v21 = main_table_is_confirmed_v21(sec, expected_name)

    # v22: rule/familyも含む複数配合パターン解析
    multi_pattern_v22 = parse_main_multi_patterns_v22(
        sec, expected_no, expected_name, name_to_no, source_url
    )
    dohk_probe_v22 = probe_dohk_v22(sec, expected_name, source_url)

    dom_probe_v17 = probe_dom_around_heading_v17(sec, expected_no, expected_name, source_url)
    group_a_probe_v19 = probe_group_a_v19(sec, expected_no, expected_name, name_to_no, source_url)
    group_b_probe_v19 = probe_group_b_v19(sec, expected_no, expected_name, name_to_no, source_url)
    group_c_probe_v19 = probe_group_c_v19(sec, expected_no, expected_name, name_to_no, source_url)
    ...
    # v21: v19の解析フローを維持しつつ、主配合表候補を統合
    for cand in main_candidates_v21:
        if not any(same_recipe_ordered(cand, x) for x in found):
            found.append(cand)

    # v22: 複数パターン型のrule/family対応候補も統合
    for cand in multi_pattern_v22:
        if not any(same_recipe_ordered(cand, x) for x in found):
            found.append(cand)
    ...
    # v21: 主配合表で確定できている場合のみ、主対象用fallbackを除去。
    # ROUTE / DIRECT / 4体配合など既存の正規候補は一切削除しない。
    if main_confirmed_v21:
        found = [x for x in found if not getattr(x, "is_fallback", False)]
    ...
    return found, {
        ...
        "group_a_probe_v19": group_a_probe_v19,
        "group_b_probe_v19": group_b_probe_v19,
        "group_c_probe_v19": group_c_probe_v19,
        "main_confirmed_v21": main_confirmed_v21,
        "main_candidate_count_v21": len(main_candidates_v21),
        "fallback_suppressed_v21": main_confirmed_v21,
        "multi_pattern_v22": [
            {
            ...
            for x in multi_pattern_v22
        ],
        "dohk_probe_v22": dohk_probe_v22,
        "dohk_dom_probe_v23": dohk_dom_probe_v23,
        "dohk_table_probe_v23": dohk_table_probe_v23,
        "dohk_page_trace_v24": dohk_page_trace_v24,
        "dohk_heading_trace_v24": dohk_heading_trace_v24,
...
# ============================================================
# v26: 最終クリーンアップ
# - 親欄へ見出し/入手方法ラベルが混入した観測を除外
# - signature単位で1レシピへ統合
# - 出典は provenance CSV に複数保持
# ============================================================

def malformed_parent_reason_v26(r: dict) -> str:
    parents = [clean_text(r.get(f"parent{i}", "")) for i in range(1, 5)]
    for p in parents:
        if not p:
            continue
        if "入手方法" in p:
            return "親欄に入手方法テキスト混入"
        # 親名/系統/ruleそのものに記事ラベル括弧が残るのは構造切り出し失敗
        if "【" in p or "】" in p:
            return "親欄に記事ラベル混入"
    return ""


def source_priority_v26(r: dict):
    """
    同一signatureの代表行を選ぶ優先度。
    DIRECTを優先し、その後ROUTE、fallbackは最後。
    """
    method = clean_text(r.get("source_method", ""))
    context = clean_text(r.get("source_context", ""))
    is_fb = str(r.get("is_fallback", "")).lower() in ("true", "1", "yes")

    if is_fb:
        p = 9
    elif "DIRECT" in method or context == "DIRECT":
        p = 0
    elif "ROUTE" in method or context == "ROUTE":
        p = 1
    else:
        p = 2
    ...
    return (
        p,
        0 if clean_text(r.get("confidence","")) == "HIGH" else 1,
        clean_text(r.get("source_url","")),
    )


def cleanup_and_merge_v26(rows: list[dict]):
    """
    returns:
      master_rows: signatureごとの代表1行
      rejected_rows: malformedとして除外した観測
      provenance_rows: 採用signatureごとの全出典
      stats: 件数
    """
    rejected = []
    valid = []
    ...
    for r in rows:
        reason = malformed_parent_reason_v26(r)
        if reason:
            rr = dict(r)
            rr["v26_reject_reason"] = reason
            rejected.append(rr)
        else:
            valid.append(r)
    ...
    by_sig = {}
    for r in valid:
        by_sig.setdefault(r.get("signature",""), []).append(r)

    master_rows = []
    provenance_rows = []

    for sig, group in by_sig.items():
        group_sorted = sorted(group, key=source_priority_v26)
        ...
        for g in group_sorted:
            u = clean_text(g.get("source_url",""))
            if u and u not in urls:
                urls.append(u)
            m = clean_text(g.get("source_method",""))
            if m and m not in methods:
                methods.append(m)
            c = clean_text(g.get("source_context",""))
            if c and c not in contexts:
                contexts.append(c)
            n = clean_text(g.get("note",""))
            if n and n not in notes:
                notes.append(n)

        rep["v26_source_count"] = len(group_sorted)
        rep["v26_source_url_count"] = len(urls)
        rep["v26_all_source_urls"] = " || ".join(urls)
        rep["v26_all_source_methods"] = " || ".join(methods)
        rep["v26_all_source_contexts"] = " || ".join(contexts)
        master_rows.append(rep)

        provenance_rows.append({
            "signature": sig,
            "result_no": rep.get("result_no",""),
            "result_name": rep.get("result_name",""),
            "recipe_type": rep.get("recipe_type",""),
            "parent1": rep.get("parent1",""),
            "parent2": rep.get("parent2",""),
            "parent3": rep.get("parent3",""),
            "parent4": rep.get("parent4",""),
            "source_observation_count": len(group_sorted),
            "unique_source_url_count": len(urls),
            "source_urls": " || ".join(urls),
            "source_methods": " || ".join(methods),
            "source_contexts": " || ".join(contexts),
            "notes": " || ".join(notes),
        })

    master_rows.sort(key=lambda r: (
        int(r["result_no"]) if str(r.get("result_no","")).isdigit() else 9999,
        r.get("signature",""),
    ))
    ...
    provenance_rows.sort(key=lambda r: (
        int(r["result_no"]) if str(r.get("result_no","")).isdigit() else 9999,
        r.get("signature",""),
    ))

    stats = {
        "observation_count_before": len(rows),
        "rejected_malformed_count": len(rejected),
        "valid_observation_count": len(valid),
        "unique_recipe_count": len(master_rows),
        "duplicate_observation_count": len(valid) - len(master_rows),
    }
    return master_rows, rejected, provenance_rows, stats
...
def semantic_parent_issues_v27(r: dict):
    issues = []
    for i in range(1, 5):
        typ = clean_text(r.get(f"parent{i}_type", ""))
        val = clean_text(r.get(f"parent{i}", ""))
        if not val:
            continue
        if val in ("親A", "親B"):
            issues.append((i, "PLACEHOLDER", val))
        elif typ == "rule" and ("or" in val.lower() or "(" in val or ")" in val):
            issues.append((i, "BROKEN_OR_EXPRESSION", val))
        elif typ == "rule" and val.endswith("など"):
            issues.append((i, "ETC_EXPRESSION", val))
        elif typ == "rule" and " " in val:
            issues.append((i, "SPLIT_NAME_OR_TEXT", val))
        elif typ == "rule" and val.endswith("系"):
            issues.append((i, "FAMILY_AS_RULE", val))
    return issues


def semantic_cleanup_v27(rows: list[dict]):
    accepted, rejected, review, normalized = [], [], [], []

    for src in rows:
        r = dict(src)
        issues = semantic_parent_issues_v27(r)
        hard = [x for x in issues if x[1] in ("PLACEHOLDER", "BROKEN_OR_EXPRESSION")]
        soft = [x for x in issues if x[1] in ("ETC_EXPRESSION", "SPLIT_NAME_OR_TEXT")]
        families = [x for x in issues if x[1] == "FAMILY_AS_RULE"]

        if hard:
            r["v27_action"] = "REJECT"
            r["v27_reason"] = " || ".join(f"parent{i}:{kind}:{val}" for i, kind, val in hard)
            rejected.append(r)
            continue

        if soft:
            r["v27_action"] = "REVIEW"
            r["v27_reason"] = " || ".join(f"parent{i}:{kind}:{val}" for i, kind, val in soft)
            review.append(r)
            continue

        if families:
            for i, _, val in families:
                r[f"parent{i}_type"] = "family"
            r["v27_action"] = "NORMALIZE_FAMILY"
            r["v27_reason"] = " || ".join(f"parent{i}:rule→family:{val}" for i, _, val in families)
            normalized.append(dict(r))

        accepted.append(r)

    # family正規化後のsignatureを再生成
    for r in accepted:
        parts = [str(r.get("result_no", "")), str(r.get("recipe_type", ""))]
        for i in range(1, 5):
            parts += [
                clean_text(r.get(f"parent{i}_type", "")),
                clean_text(r.get(f"parent{i}_no", "")),
                clean_text(r.get(f"parent{i}", "")),
            ]
        r["signature"] = "|".join(parts)

    merged, _, provenance, _ = cleanup_and_merge_v26(accepted)

    stats = {
        "v26_master_count": len(rows),
        "v27_rejected_count": len(rejected),
        "v27_review_count": len(review),
        "v27_family_normalized_count": len(normalized),
        "v27_final_master_count": len(merged),
    }
    return merged, rejected, review, normalized, provenance, stats
...
def semantic_false_positive_audit_v28(rows: list[dict]) -> list[dict]:
    """
    v27正式候補を変更せず、意味的な誤結合の疑いだけをREVIEW候補へ出す。

    既知の基準例:
      はぐれメタルキング <- はぐれメタル x はぐれメタル
    は、同ページ内の別領域（「使う主な配合」/4体配合等）由来の
    誤結合と確認されたため必ず検出する。
    ...
    for src in rows:
        r = dict(src)
        reasons = []
        result = clean_text(r.get("result_name", ""))
        p1 = clean_text(r.get("parent1", ""))
        p2 = clean_text(r.get("parent2", ""))
        typ = clean_text(r.get("recipe_type", ""))
        ctx = clean_text(r.get("source_context", ""))
        method = clean_text(r.get("source_method", ""))
        raw = clean_text(r.get("raw_text", ""))
        all_ctx = clean_text(r.get("v26_all_source_contexts", ""))
        all_methods = clean_text(r.get("v26_all_source_methods", ""))

        # 今回、人手確認で偽陽性と確定した基準例。
        if (result == "はぐれメタルキング" and typ == "2体配合"
                and p1 == "はぐれメタル" and p2 == "はぐれメタル"):
            reasons.append("KNOWN_FALSE_POSITIVE: はぐれメタル×4→メタルキング等の周辺情報との誤結合")

        # DIRECTの2体配合なのに、raw_textに別意味領域の明示語が残るもの。
        # 正常例もあり得るためREVIEW止まり。
        if typ == "2体配合" and (ctx == SOURCE_DIRECT or SOURCE_DIRECT in all_ctx):
            boundary_words = [
                "使う主な配合", "4体配合", "までの作成ルート",
                "【入手方法】", "スカウト", "他国マスター",
            ]
            hit = [w for w in boundary_words if w in raw]
            if hit:
                reasons.append("DIRECT_CROSS_CONTEXT_TEXT: " + "/".join(hit))

        # 同一親×同一親は正規レシピも存在するため削除不可。
        # ただしDIRECT由来かつ根拠が1URLのみなら今回型の監査対象にする。
        if (typ == "2体配合" and p1 and p1 == p2
                and (ctx == SOURCE_DIRECT or SOURCE_DIRECT in all_ctx)):
            try:
                url_count = int(r.get("v26_source_url_count", 0) or 0)
            except Exception:
                url_count = 0
            if url_count <= 1:
                reasons.append("SAME_PARENT_DIRECT_SINGLE_SOURCE")

        # fallback/UNKNOWNが混ざるDIRECT候補は、構造化DIRECT単独より弱い。
        if typ == "2体配合" and (ctx == SOURCE_DIRECT or SOURCE_DIRECT in all_ctx):
            weak = any(x in (method + " " + all_methods) for x in (
                METHOD_TEXT_FALLBACK, METHOD_LINK_ANY_FALLBACK, METHOD_UNKNOWN
            ))
            if weak:
                reasons.append("DIRECT_WITH_WEAK_PROVENANCE")

        if reasons:
            r["v28_action"] = "REVIEW"
            r["v28_reason"] = " || ".join(dict.fromkeys(reasons))
            out.append(r)

    out.sort(key=lambda r: (
        int(r.get("result_no", 9999)) if str(r.get("result_no", "")).isdigit() else 9999,
        r.get("signature", ""),
    ))
    return out
...
# ============================================================
# v28改2: 人手確定した偽陽性を、根拠ベースの一般ルールで除外
# ============================================================
def _adjacent_token_pair(raw: str, token: str) -> bool:
    """raw_text の空白区切りトークン上で token token が隣接するか。"""
    toks = [clean_text(x) for x in (raw or "").split() if clean_text(x)]
    return any(toks[i] == token and toks[i + 1] == token for i in range(len(toks) - 1))


def semantic_false_positive_cleanup_v28_kai(rows: list[dict]):
    """
    v28/v28改の人手検証結果を一般化して反映する。

    1) SAME_PARENT_NO_RAW_PAIR
       v13 ADDITIONAL direct 由来の「同一親×同一親」候補について、
       raw_text に同一親名が隣接して2個並ぶ根拠が無ければ REJECT。

       これで、結果名や周辺テキスト中の短いモンスター名を誤って2回拾った
       偽陽性を除外する。

       人手確認で正常だった例:
         ヘルバトラー <- アンクルホーン×アンクルホーン
         スライムジェネラル <- メタルカイザー×メタルカイザー
       は raw_text に実際に「親名 親名」が並ぶので KEEP される。

       人手確認で偽陽性だった例:
         グレイトドラゴン <- ドラゴン×ドラゴン
         魔王デスタムーア <- デスタムーア×デスタムーア
         はぐれメタルキング <- はぐれメタル×はぐれメタル
       等は raw_text に隣接同一親の根拠が無いため REJECT される。

    2) EXPLANATORY_FAMILY_CROSS_CONTEXT
       系統指定親を含む v13 ADDITIONAL direct 候補で、系統名が
       「別モンスターは○○と△△系の配合で生まれる」等の説明文にのみ現れる
       場合は、ページ主役の配合へ誤帰属したものとして REJECT。

       既知例:
         ヒヒュルデの使い <- ポンポコあにき×ゾンビ系
       実際は コングヘッド <- キラーエイプ×ゾンビ系 の説明文。

    自動除外は上記の構造条件を満たすものに限定する。
    """
    kept, rejected = [], []
    for src in rows:
        r = dict(src)
        ...
        p1 = clean_text(r.get("parent1", ""))
        p2 = clean_text(r.get("parent2", ""))
        p1_type = clean_text(r.get("parent1_type", ""))
        p2_type = clean_text(r.get("parent2_type", ""))
        typ = clean_text(r.get("recipe_type", ""))
        ...
        # ---- 同一親×同一親の誤生成 ----
        # v13 ADDITIONAL direct の候補に限定し、raw_text に親名×2の
        # 隣接トークン根拠がある正常ケースは必ず残す。
        if (
            typ == "2体配合"
            and "v13 ADDITIONAL direct / 2体配合" in note
            and p1 and p1 == p2
            and not _adjacent_token_pair(raw, p1)
        ):
            reasons.append("SAME_PARENT_NO_RAW_PAIR")

        # ---- 説明文中の別世代配合をページ主役へ誤帰属 ----
        family = ""
        concrete = ""
        if p1_type == "family" and p2_type != "family":
            family, concrete = p1, p2
        elif p2_type == "family" and p1_type != "family":
            family, concrete = p2, p1

        if (
            typ == "2体配合"
            and "v13 ADDITIONAL direct / 系統指定親" in note
            and family and concrete
            and family in raw
            and concrete in raw
            and ("配合で生まれる" in raw or "配合で入手" in raw)
            and ("スカウト" in raw or "入手" in raw)
        ):
            # 説明文の「XはYと○○系の配合で…」型。
            # family が主配合表ではなく説明文側に現れるケースを除外する。
            fam_pos = raw.find(family)
            explain_pos_candidates = [p for p in (raw.find("配合で生まれる"), raw.find("配合で入手")) if p >= 0]
            explain_pos = min(explain_pos_candidates) if explain_pos_candidates else -1
            if fam_pos >= 0 and explain_pos >= 0 and fam_pos < explain_pos:
                # さらに説明文より前に、具体親とは別のモンスター名が配合表側に
                # 置かれていることを要求し、通常の family 配合を巻き込まない。
                # ヒヒュルデの使いでは「ポンポコあにき コングヘッド」が先にある。
                before = raw[:fam_pos]
                if concrete in before and "は" in before:
                    reasons.append("EXPLANATORY_FAMILY_CROSS_CONTEXT")

        if reasons:
            r["v28k_action"] = "REJECT"
            r["v28k_reason"] = " || ".join(dict.fromkeys(reasons))
            rejected.append(r)
        else:
            r["v28k_action"] = "KEEP"
            r["v28k_reason"] = ""
...
def final_recipe_key(r: dict) -> tuple:
    """最終重複検出用。2体配合は親順不同、4体配合は原順序保持。"""
    result_no = int(clean_text(r.get("result_no", "")) or 0)
    typ = clean_text(r.get("recipe_type", ""))
    parents = []
    for i in range(1, 5):
        p = (
...
def rebuild_signature_final(r: dict) -> str:
    parts = [clean_text(r.get("result_no", "")), clean_text(r.get("recipe_type", ""))]
    for i in range(1, 5):
        parts.extend([
            clean_text(r.get(f"parent{i}_type", "")),
            clean_text(r.get(f"parent{i}_no", "")),
...
def normalize_spaced_v27_review_row(src: dict) -> dict | None:
    """v27 REVIEWのうち、確認済み空白名だけを正式名へ正規化して復帰。"""
    r = dict(src)
    changed = False
    for i in range(1, 5):
        val = clean_text(r.get(f"parent{i}", ""))
    ...
    for i in range(1, 5):
        typ = clean_text(r.get(f"parent{i}_type", ""))
        val = clean_text(r.get(f"parent{i}", ""))
        if val and typ == "rule" and val != "相手問わず":
            unresolved.append((i, val))
    if not changed or unresolved:
        return None

    r["signature"] = rebuild_signature_final(r)
    r["final_action"] = "RESTORE"
    r["final_reason"] = "確認済み空白入り図鑑名を正式モンスター名へ正規化"
    return r


def same_parent_keep_key_final(r: dict) -> tuple[int, str, str] | None:
    try:
        no = int(clean_text(r.get("result_no", "")))
    except (TypeError, ValueError):
        return None
    p1 = clean_text(r.get("parent1", ""))
    p2 = clean_text(r.get("parent2", ""))
    if p1 and p1 == p2:
        return (no, clean_text(r.get("result_name", "")), p1)
    return None


def normalize_final_confirmed_rules(rows: list[dict]) -> tuple[list[dict], list[dict]]:
    """
    最終段階で人手確認済みのrule表記だけを安全に正規化する。
    単独「2」（スライムジェネラル）は「メタルカイザー×2」分解残骸として除外。
    """
    out, rejected = [], []
    for src in rows:
        r = dict(src)
        ...
        for i in range(1, 5):
            ...
            if val in CONFIRMED_RULE_MONSTER_MAP:
                ...
                continue
            if val == "2" and clean_text(r.get("result_name", "")) == "スライムジェネラル":
        ...
        if changed:
            r["final_reason"] += " / 最終正規化: " + " ; ".join(changed)
        out.append(r)
    return out, rejected


def finalize_confirmed_master(
    base_rows: list[dict],
    semantic_review_v27: list[dict],
    semantic_rejected_v27: list[dict],
    semantic_rejected_v28k: list[dict],
    semantic_fp_review_v28: list[dict],
) -> tuple[list[dict], dict, list[dict], list[dict], list[dict]]:
    """v26〜v28改2の結果へ人手確認済み最終判断を反映する。"""
    errors = []
    warnings = []
    base_count = len(base_rows)

    if base_count != EXPECTED_V28K2_BASE_COUNT:
        errors.append(f"v28改2基準マスター件数が想定外: {base_count} (期待 {EXPECTED_V28K2_BASE_COUNT})")
    if len(semantic_rejected_v27) != EXPECTED_V27_REJECT_COUNT:
        warnings.append(f"v27 rejected件数が想定外: {len(semantic_rejected_v27)} (期待 {EXPECTED_V27_REJECT_COUNT})")
    if len(semantic_rejected_v28k) != EXPECTED_V28K2_REJECT_COUNT:
        errors.append(f"v28改2 rejected件数が想定外: {len(semantic_rejected_v28k)} (期待 {EXPECTED_V28K2_REJECT_COUNT})")

    review_keys = {same_parent_keep_key_final(r) for r in semantic_fp_review_v28}
    review_keys.discard(None)
    if review_keys != CONFIRMED_SAME_PARENT_KEEP:
        missing = CONFIRMED_SAME_PARENT_KEEP - review_keys
        extra = review_keys - CONFIRMED_SAME_PARENT_KEEP
        if missing:
            errors.append("v28 KEEP確認18件の不足: " + ", ".join(map(str, sorted(missing))))
        if extra:
            errors.append("v28 reviewに未確認の追加候補: " + ", ".join(map(str, sorted(extra))))

    base_same = {same_parent_keep_key_final(r) for r in base_rows}
    base_same.discard(None)
    missing_from_base = CONFIRMED_SAME_PARENT_KEEP - base_same
    if missing_from_base:
        errors.append("確認済み同一親配合が基準マスターにありません: " + ", ".join(map(str, sorted(missing_from_base))))

    restored = []
    excluded_review = []
    for r in semantic_review_v27:
        x = normalize_spaced_v27_review_row(r)
        if x is not None:
            restored.append(x)
        else:
            y = dict(r)
            y["final_action"] = "EXCLUDE"
            if "ETC_EXPRESSION" in clean_text(r.get("v27_reason", "")):
    ...
    if len(restored) != EXPECTED_RESTORED_COUNT:
        errors.append(f"空白名正規化で復帰した件数が想定外: {len(restored)} (期待 {EXPECTED_RESTORED_COUNT})")

    normalized_base, final_rule_rejected = normalize_final_confirmed_rules(base_rows)

    final_rows = []
    seen = set()
    for r in normalized_base:
        k = final_recipe_key(r)
        if k in seen:
            continue
        seen.add(k)
        final_rows.append(r)
    for r in restored:
        k = final_recipe_key(r)
        if k in seen:
            warnings.append(f"復帰候補が既存マスターと重複: {r.get('result_name')} {r.get('parent1')}×{r.get('parent2')}")
            continue
        seen.add(k)
        final_rows.append(r)

    final_rows.sort(key=lambda r: (
        int(clean_text(r.get("result_no", "")) or 9999),
        clean_text(r.get("recipe_type", "")),
        clean_text(r.get("parent1", "")), clean_text(r.get("parent2", "")),
        clean_text(r.get("parent3", "")), clean_text(r.get("parent4", "")),
    ))

    if len(final_rows) != EXPECTED_FINAL_COUNT:
        errors.append(f"最終件数が想定外: {len(final_rows)} (期待 {EXPECTED_FINAL_COUNT})")

    for r in final_rows:
        for i in range(1, 5):
            typ = clean_text(r.get(f"parent{i}_type", ""))
            name = clean_text(r.get(f"parent{i}", ""))
            no = clean_text(r.get(f"parent{i}_no", ""))
            if typ == "monster" and name and not no:
                errors.append(f"monster親なのにNoなし: {r.get('result_name')} parent{i}={name}")
            if typ == "rule" and name not in ("", "相手問わず", "神獣", "神獣モンスター"):
                errors.append(f"未解決ruleが最終データに残る: {r.get('result_name')} parent{i}={name}")
    ...
    if errors:
        ...
        for w in warnings:
            print("WARNING:", w)
        raise RuntimeError("最終確定条件を満たさなかったため出力を停止しました。")

    v28_keep_rows = []
    for r in semantic_fp_review_v28:
        x = dict(r)
        x["final_action"] = "KEEP"
    ...
    stats = {
        "v28k2_base_master": base_count,
        "v27_review_total": len(semantic_review_v27),
        "v27_review_restored_spaced_names": len(restored),
        "v27_review_excluded_etc_or_other": len(excluded_review),
        "v27_rejected_kept_excluded": len(semantic_rejected_v27),
        "v28k2_false_positive_rejected_kept_excluded": len(semantic_rejected_v28k),
        "v28_same_parent_review_confirmed_keep": len(v28_keep_rows),
        "final_rule_fragment_rejected": len(final_rule_rejected),
        "final_recipe_count": len(final_rows),
        "warnings": len(warnings),
        "status": "OK",
    }
    return final_rows, stats, restored, excluded_review, final_rule_rejected


def _runtime_self_check_log():
    """GitHub Actionsで実際に実行されているPython本体を識別するための診断ログ。"""
    import hashlib
    try:
        p = Path(__file__).resolve()
        raw = p.read_bytes()
        sha = hashlib.sha256(raw).hexdigest()
        print("\n===== RUNTIME SELF CHECK =====", flush=True)
        print(f"EXECUTED_FILE: {p}", flush=True)
        print(f"FILE_SIZE: {len(raw)} bytes", flush=True)
        print(f"SHA256: {sha}", flush=True)
        print("EXPECTED_BUILD_TAG: FINAL_INTEGRATED_LOGCHECK_V2_TYPESAFE", flush=True)
        try:
            head = p.read_text(encoding="utf-8", errors="replace").splitlines()[:30]
            print("FILE_HEAD_BEGIN", flush=True)
            for line in head:
                print(line, flush=True)
            print("FILE_HEAD_END", flush=True)
        except Exception as e:
            print(f"FILE_HEAD_ERROR: {e}", flush=True)

        # 最終統合版に必須の文字列が本体に存在するか確認
        body = p.read_text(encoding="utf-8", errors="replace")
        required = [
            "semantic_cleanup_v27",
            "semantic_false_positive_cleanup_v28_kai",
            "breeding.csv",
            "altema_breeding_final_full.csv",
            "final_extract_summary.csv",
        ]
        for key in required:
            print(f"REQUIRED_TOKEN[{key}]: {'YES' if key in body else 'NO'}", flush=True)
        print("===== END RUNTIME SELF CHECK =====\n", flush=True)
    except Exception as e:
        print(f"RUNTIME_SELF_CHECK_ERROR: {type(e).__name__}: {e}", flush=True)


def main():
    _runtime_self_check_log()
    ...
    group_a_rows_v19: list[dict] = []
    group_b_rows_v19: list[dict] = []
    group_c_rows_v19: list[dict] = []
    multi_pattern_rows_v22: list[dict] = []
    dohk_probe_rows_v22: list[dict] = []
    dohk_dom_rows_v23: list[dict] = []
    dohk_table_rows_v23: list[dict] = []
    dohk_page_trace_rows_v24: list[dict] = []
    dohk_heading_trace_rows_v24: list[dict] = []
    ...
    for no in range(1, 660):
        ...
        try:
            ...
            group_a_rows_v19.extend(meta.get("group_a_probe_v19", []))
            group_b_rows_v19.extend(meta.get("group_b_probe_v19", []))
            group_c_rows_v19.extend(meta.get("group_c_probe_v19", []))
            multi_pattern_rows_v22.extend(meta.get("multi_pattern_v22", []))
            dohk_probe_rows_v22.extend(meta.get("dohk_probe_v22", []))
            dohk_dom_rows_v23.extend(meta.get("dohk_dom_probe_v23", []))
            dohk_table_rows_v23.extend(meta.get("dohk_table_probe_v23", []))
            dohk_page_trace_rows_v24.extend(meta.get("dohk_page_trace_v24", []))
            dohk_heading_trace_rows_v24.extend(meta.get("dohk_heading_trace_v24", []))
            ...
            if meta["status"] not in ("OK", "NO_BREEDING"):
                failures.append({
                    "no": no,
                    "name": name,
    ...
    # 個別ページ取得結果
    # ---- v26: 最終クリーンアップを本出力の直前に実施 ----
    observations_v26 = list(all_rows)
    all_rows, rejected_v26, provenance_v26, stats_v26 = cleanup_and_merge_v26(
        observations_v26
    )

    # 元の全観測も監査用に保存
    observation_fields_v26 = [
        "result_no","result_name","result_rank","result_family","result_size",
        "recipe_type",
        "parent1_type","parent1_no","parent1",
        "parent2_type","parent2_no","parent2",
        "parent3_type","parent3_no","parent3",
        "parent4_type","parent4_no","parent4",
        "confidence","note","source_context","source_method",
        "is_fallback","fallback_reason","source","source_url","source_page",
        "signature","raw_text",
    ]
    write_csv(
        OUT / "altema_breeding_v26_all_observations.csv",
        observations_v26,
        observation_fields_v26,
    )

    write_csv(
        OUT / "altema_breeding_v26_rejected_malformed.csv",
        rejected_v26,
        observation_fields_v26 + ["v26_reject_reason"],
    )

    write_csv(
        OUT / "altema_breeding_v26_provenance.csv",
        provenance_v26,
        [
            "signature","result_no","result_name","recipe_type",
            "parent1","parent2","parent3","parent4",
            "source_observation_count","unique_source_url_count",
            "source_urls","source_methods","source_contexts","notes",
        ],
    )

    write_csv(
        OUT / "altema_breeding_v26_cleanup_summary.csv",
        [stats_v26],
        [
            "observation_count_before","rejected_malformed_count",
            "valid_observation_count","unique_recipe_count",
    ...
    (
        all_rows,
        semantic_rejected_v27,
        semantic_review_v27,
        semantic_normalized_v27,
        provenance_semantic_v27,
        semantic_stats_v27,
    ) = semantic_cleanup_v27(all_rows)
    ...
    semantic_fields_v27 = [
        ...
        "confidence","note","source_context","source_method",
        "is_fallback","fallback_reason","source","source_url","source_page",
        "signature","raw_text","v27_action","v27_reason",
    ]

    write_csv(OUT / "altema_breeding_v27_semantic_rejected.csv",
              semantic_rejected_v27, semantic_fields_v27)
    write_csv(OUT / "altema_breeding_v27_semantic_review.csv",
              semantic_review_v27, semantic_fields_v27)
    write_csv(OUT / "altema_breeding_v27_family_normalized.csv",
              semantic_normalized_v27, semantic_fields_v27)
    write_csv(
        OUT / "altema_breeding_v27_cleanup_summary.csv",
        [semantic_stats_v27],
        ["v26_master_count","v27_rejected_count","v27_review_count",
         "v27_family_normalized_count","v27_final_master_count"],
    )

    # ---- v28改2: 人手確定A/C群＋説明文誤帰属を一般ルールで除外 ----
    all_rows, semantic_rejected_v28k = semantic_false_positive_cleanup_v28_kai(all_rows)
    semantic_v28k_fields = semantic_fields_v27 + ["v28k_action", "v28k_reason"]
    write_csv(
        OUT / "altema_breeding_v28k2_semantic_rejected.csv",
        semantic_rejected_v28k,
        semantic_v28k_fields,
    )
    write_csv(
        OUT / "altema_breeding_v28k2_cleanup_summary.csv",
        [{
            "v27_master_count": semantic_stats_v27["v27_final_master_count"],
            "v28k2_rejected_count": len(semantic_rejected_v28k),
            "v28k2_final_master_count": len(all_rows),
        }],
        ["v27_master_count", "v28k2_rejected_count", "v28k2_final_master_count"],
    )

    # ---- v28監査は、v28改2で確定偽陽性を除外した後の候補に対して継続 ----
    semantic_fp_review_v28 = semantic_false_positive_audit_v28(all_rows)
    semantic_fp_fields_v28 = semantic_fields_v27 + [
        "v26_source_count","v26_source_url_count","v26_all_source_urls",
        "v26_all_source_methods","v26_all_source_contexts",
        "v28_action","v28_reason",
    ]
    write_csv(
        OUT / "altema_breeding_v28_false_positive_review.csv",
        semantic_fp_review_v28,
        semantic_fp_fields_v28,
    )
    known_fp_found = any(
        clean_text(r.get("result_name", "")) == "はぐれメタルキング"
        and clean_text(r.get("parent1", "")) == "はぐれメタル"
        and clean_text(r.get("parent2", "")) == "はぐれメタル"
        for r in semantic_fp_review_v28
    )
    write_csv(
        OUT / "altema_breeding_v28_audit_summary.csv",
        [{
            "v27_master_count": len(all_rows),
            "v28_review_candidate_count": len(semantic_fp_review_v28),
            "v28_master_count_unchanged": len(all_rows),
            "known_hagure_false_positive_detected": "YES" if known_fp_found else "NO",
        }],
        ["v27_master_count","v28_review_candidate_count",
         "v28_master_count_unchanged","known_hagure_false_positive_detected"],
    )

    # ---- 最終確定: v26〜v27仕様の同一実行内で最終マスターまで作る ----
    all_rows, final_stats, final_restored, final_excluded_review, final_rule_rejected = finalize_confirmed_master(
        all_rows,
        semantic_review_v27,
        semantic_rejected_v27,
        semantic_rejected_v28k,
    ...
    final_audit_fields = semantic_fp_fields_v28 + ["final_action", "final_reason"]
    # DictWriterのextrasaction=ignoreで重複/余剰列は安全に無視される。
    write_csv(OUT / "final_restored_from_v27_review.csv", final_restored, final_audit_fields)
    write_csv(OUT / "final_excluded_v27_review.csv", final_excluded_review, final_audit_fields)
    write_csv(OUT / "final_rule_normalization_rejected.csv", final_rule_rejected, final_audit_fields)
    ...
    for r in semantic_fp_review_v28:
        ...
        x["final_action"] = "KEEP"
        x["final_reason"] = "スクリーンショット確認済み正常な同一親×同一親"
        final_v28_keep.append(x)
    write_csv(OUT / "final_v28_review_kept.csv", final_v28_keep, final_audit_fields)

    write_csv(
        OUT / "final_extract_summary.csv",
        [{"item": k, "value": str(v)} for k, v in final_stats.items()],
        ["item", "value"],
    )

    # アプリ投入用の最終コアCSVも同じ output_breeding/ に出す。
    write_csv(OUT / "breeding.csv", all_rows, FINAL_CORE_FIELDS)
    ...
    write_csv(
        OUT / "altema_breeding_final_full.csv",
    ...
    # ---- v24: /monster/564 取得→見出し→section トレース ----
    write_csv(
        OUT / "altema_breeding_v24_dohk_page_trace.csv",
        dohk_page_trace_rows_v24,
        [
            "requested_source_url","page_no","title","html_length","page_text_length",
            "contains_dohk_html","contains_dohk_text",
            "contains_creation_heading_html","contains_creation_heading_text",
            "contains_akubar_html","contains_baruzakku_html","contains_mudo_html",
            "contains_kandata_oyabun_html","contains_kandata_wife_html",
            "contains_other_patterns_html","contains_mojibake_hint",
            "heading_found","heading_tag","heading_class","heading_id","heading_text",
            "section_length",
            "section_contains_akubar","section_contains_baruzakku","section_contains_mudo",
            "section_contains_kandata_oyabun","section_contains_kandata_wife",
            "section_contains_other_patterns",
            "html_prefix","page_text_prefix","section_text_prefix"
        ],
    )
    write_csv(
        OUT / "altema_breeding_v24_dohk_heading_trace.csv",
        dohk_heading_trace_rows_v24,
        [
            "result_name","candidate_index","tag","class","id","text",
            "parent_tag","parent_class","parent_id","parent_text","source_url"
        ],
    )

    # ---- v23: ドーク専用DOM診断 ----
    write_csv(
        OUT / "altema_breeding_v23_dohk_dom_probe.csv",
        dohk_dom_rows_v23,
        [
            "result_name","matched_keywords","tag","class","id","text","href","img_alt",
            "parent_tag","parent_class","parent_id","parent_text",
            "grandparent_tag","grandparent_class","grandparent_id","grandparent_text",
    ...
    write_csv(
        OUT / "altema_breeding_v23_dohk_table_probe.csv",
        dohk_table_rows_v23,
        [
            "result_name","table_index","table_class","table_id","table_matched_keywords",
            "row_index","row_text","cell_count","cell_texts","link_texts","img_alts","source_url"
        ],
    )

    # ---- v22: 最終例外検証 ----
    write_csv(
        OUT / "altema_breeding_v22_multi_pattern_validation.csv",
        multi_pattern_rows_v22,
        ["result_name","parent1","parent2","note","raw_text"],
    )

    write_csv(
        OUT / "altema_breeding_v22_dohk_probe.csv",
        dohk_probe_rows_v22,
        [
            "result_name","table_index",
            "has_other_patterns","has_pattern",
            "table_text","source_url"
        ],
    )

    # 配合なし3体 + ドーク + プチットガールズの最終確認
    v22_focus = {
        "スライム","ズッキーニャ","キラーピッケル",
        "ドーク","プチットガールズ"
    }
    v22_focus_rows = []
    for r in all_rows:
        if r.get("result_name") in v22_focus:
            v22_focus_rows.append({
                "result_no": r.get("result_no",""),
                "result_name": r.get("result_name",""),
                "recipe_type": r.get("recipe_type",""),
                "parent1": r.get("parent1",""),
    ...
    write_csv(
        OUT / "altema_breeding_v22_focus_validation.csv",
        v22_focus_rows,
        [
    ...
    # ---- v21: 合格判定用監査 ----
    v21_focus_names = {
        "コドラ","ミステリドール","プチットガールズ",
        "モヒカント","キラーパンサー","トラップボックス",
    }
    v21_focus_rows = []
    for r in all_rows:
        if r.get("result_name") in v21_focus_names:
            v21_focus_rows.append({
                "result_no": r.get("result_no",""),
                "result_name": r.get("result_name",""),
                "recipe_type": r.get("recipe_type",""),
                "parent1": r.get("parent1",""),
    ...
    write_csv(
        OUT / "altema_breeding_v21_focus6_validation.csv",
        v21_focus_rows,
        [
            "result_no","result_name","recipe_type",
            "parent1","parent2","parent3","parent4",
    ...
    # ---- v19: 残り6件の3分類診断 ----
    write_csv(
        OUT / "altema_breeding_v19_groupA_normal2body_probe.csv",
        group_a_rows_v19,
        [
            "group","result_no","result_name","table_index","row_index",
            "table_class","row_tag","row_class","row_text",
            "known_monsters","family","any_partner",
            "link_count","img_count","source_url"
        ],
    )

    write_csv(
        OUT / "altema_breeding_v19_groupB_multi_pattern_probe.csv",
        group_b_rows_v19,
        [
            "group","result_no","result_name","table_index",
            "pattern_header_row","relative_offset","row_index",
            "row_text","known_monsters","cell_count","link_count","source_url"
        ],
    )

    write_csv(
        OUT / "altema_breeding_v19_groupC_4body_fallback_probe.csv",
        group_c_rows_v19,
        [
            "group","result_no","result_name","table_index",
            "table_text","known_monsters",
            "has_4body_marker","has_any_partner","source_url"
        ],
    )

    focus6 = V19_GROUP_A | V19_GROUP_B | V19_GROUP_C
    current_rows_v19 = []
    for r in all_rows:
        if r.get("result_name") not in focus6:
            continue
        current_rows_v19.append({
            "group": classify_v19_group(r.get("result_name","")),
            "result_no": r.get("result_no",""),
            "result_name": r.get("result_name",""),
            "recipe_type": r.get("recipe_type",""),
            "parent1": r.get("parent1",""),
    ...
    write_csv(
        OUT / "altema_breeding_v19_remaining6_current_state.csv",
        current_rows_v19,
        [
            "group","result_no","result_name","recipe_type",
            "parent1","parent2","parent3","parent4",
            "confidence","source_method","is_fallback",
            "note","raw_text","source_url"
        ],
    ...
    print(f"\n出力先: {OUT}")
    print("最終確定 breeding.csv と final_extract_summary.csv を確認してください。")
    print(f"最終確定配合件数: {len(all_rows)}")
