...
# -*- coding: utf-8 -*-
"""
TerrySP / Altema 個別モンスターページ 配合再検証ツール v2
...
def split_recipe_fragments(s: str) -> list[str]:
    """
    1セル/1行に複数の配合が連結されているケースを分割する。
    【特殊配合】A×B C×D のような表記を独立候補へ。
    """
    s = normalize_rule_text(s)
    # 入手方法以降は除外
    s = strip_after_non_recipe_marker(s)
    if not s:
        return []

    # 配合種別マーカーは境界として扱うが、内容は保持
    for marker in SECTION_MARKERS:
        s = s.replace(marker, "\n")

    # 句点・改行・中黒のうち「×」を含む断片だけ候補化
    parts = re.split(r"[\n。；;]+", s)
    out = []
    for p in parts:
        p = clean_text(p)
        if "×" not in p:
            continue
        # 1断片中に複数×が並ぶ場合、モンスター/ルール名を使って後段で分解できるようそのまま保持
        out.append(p)
    return out
...
def parse_pair_from_row(
    ...
    texts = [strip_after_non_recipe_marker(node_text(c)) for c in cells]
    joined = " | ".join(texts)
    original_joined = " | ".join(normalize_rule_text(node_text(c)) for c in cells)

    # 入手方法しか書かれていないセルを親として採用しない
    if any(marker in original_joined for marker in NON_RECIPE_MARKERS) and not any("×" in t for t in texts):
        return None
...
def parse_pair_from_text_line(
    ...
    """
    テキスト断片から A × B を抽出。
    入手方法などは除外し、1断片に複数配合がある場合は可能な範囲で複数候補へ。
    """
    out = []
    for frag in split_recipe_fragments(text):
        # まず単純な A × B を抽出。括弧説明はなるべく残さない。
        # モンスター名またはルールらしいトークンを左右にとる。
        # 1行に複数ある場合は「次の×の左辺候補」までを分ける。
        pieces = [clean_text(x) for x in frag.split("×")]
        if len(pieces) < 2:
            continue
        ...
        if len(pieces) == 2:
            ...
            # A×B C×D のような連結に備え、既知モンスター名を境界に再走査
            # 安全に切れない場合は review 扱いに落とす。
            pairs.append((pieces[0], pieces[1]))
            for i in range(1, len(pieces)-1):
                left_tail = pieces[i].split()[-1] if pieces[i].split() else pieces[i]
                if left_tail in name_to_no:
                    pairs.append((left_tail, pieces[i+1]))

        for left, right in pairs:
            left = strip_after_non_recipe_marker(left)
            right = strip_after_non_recipe_marker(right)
            if not left or not right:
                continue

            # 入手方法語が残る候補は除外
            bad_words = ("入手方法", "スカウト", "タマゴ", "たまご", "孵化", "入手でき")
            if any(w in left or w in right for w in bad_words):
                continue

            p1 = classify_parent_token(left, [left] if left in name_to_no else [], name_to_no)
            p2 = classify_parent_token(right, [right] if right in name_to_no else [], name_to_no)
...
def main():
    ...
    write_csv(
        ...
        review_rows,
        ["no","name","reason","recipe","source_url","raw_text"],
    )
    ...
    acquisition_like = sum(
        1 for r in all_rows
        if any(w in (r.get("raw_text","") + " " + r.get("parent1","") + " " + r.get("parent2",""))
               for w in ("【入手方法】","スカウト","タマゴ","たまご","孵化"))
    )
